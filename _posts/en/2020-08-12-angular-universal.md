...
If we open our local [http://localhost:4200](http://localhost:4200/) we will see our usual app, BUT let's see if the magic is there, we open the source code (ctrl + u) and check the HTML. If we can see it is that it works, in fact the newest versions of universal add a comment:  *This page was prerendered with Angular Universal*.
...
So, the ***way*** in which we were implementing Universal changed. 
